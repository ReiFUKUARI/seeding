...
「どう構成するか」を具体化する（決定事項は「6. 決定事項一覧」参照）。

> **Windows実機で署名付き `.wapproj` ビルド・証明書インポート・インストール・
> 起動までの一連の流れを確認済みです。** 実機でしか再現しない環境依存の
> エラー（`APPX3217` / `NETSDK1112` / 証明書パスワード関連 / 発行元証明書が
> 未信頼 等）を複数踏んだため、「5. ビルド手順」に対処法をまとめてあります。
> 躓いたら先にそちらを確認してください。
...
検知して自動更新できる仕組みも検討したが、**最終的に不採用と確定した**
（[D-010](./decisions.md#d-010-自動更新appinstallerは不採用不具合発生時に都度手動再インストール)）。
`.wapproj` には `GenerateAppInstallerFile` 等のプロパティを一切追加していない。

不具合が見つかった場合は改修のうえ、この文書の「5. ビルド手順」
「署名ありの配布パッケージ」の手順で作り直したMSIXを、各営業担当者が
[`scripts/install-for-user.ps1`](../packaging/MailDeliveryTool.Package/scripts/install-for-user.ps1)
で個別に手動再インストールする運用とする。
...
| C | 自動更新 | **確定済み**。不採用。不具合発生時は都度改修し、各自で手動再インストール（[D-010](./decisions.md#d-010-自動更新appinstallerは不採用不具合発生時に都度手動再インストール)） |
| D | アプリアイコン | **確定済み**。モックのロゴをそのまま採用（[D-008](./decisions.md#d-008-アプリアイコンはモックのロゴをそのまま採用)） |

実機での署名付きMSIXビルド・インストール・起動確認（タイトル表示、
タスクバーアイコン、フェーズ8のUIが反映されていること）まで完了しており、
未決事項は残っていない。
